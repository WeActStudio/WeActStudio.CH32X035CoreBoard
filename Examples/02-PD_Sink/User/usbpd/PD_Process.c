...
pd_pdo_powerdata_t pd_pdo_src_powerdata[10];
uint8_t pd_index_now;
/*********************************************************************
 * @fn      USBPD_IRQHandler
 *
        ...
        switch( pd_header )
        {
            case DEF_TYPE_SRC_CAP:
                ...
                for (var = 1; var <= PDO_Len; ++var)
                {
                    ...
                    printf("PDO:%d\r\nCurrent:%d mA\r\nVoltage:%d mV\r\n",var,Current,Voltage);
                    pd_pdo_src_powerdata[var-1].current = Current;
                    pd_pdo_src_powerdata[var-1].voltage = Voltage;
                }
                printf("\r\n");
                uint8_t pdo_index = PD_Find_MaxVoltage(pd_pdo_src_powerdata,PDO_Len);
...
// 找出最大电压，并返回Index
uint8_t PD_Find_MaxVoltage(pd_pdo_powerdata_t *data,uint8_t pdo_len)
{
    uint32_t i;
    uint16_t max = data[0].voltage;
    uint8_t pdo_index = 1;
    for(i = 1;i<pdo_len;i++)
    {
        if(data[i].voltage >= max)
        {
            max = data[i].voltage;
            pdo_index = i + 1;
        }
...
// 计算功率，单位mW
uint32_t PD_Calculate_Power(pd_pdo_powerdata_t *data,uint8_t index)
{
    uint32_t power;
    if(index == 0)
    {
        ...
        power = data[index-1].current * data[index-1].voltage / 1000;
    }
    return power;
}

// 获取当前设置电压PDO的id号
uint8_t PD_Get_Index_Now(void){
    return pd_index_now;
}

// 查询电压是否申请成功
uint8_t PD_Request_Is_Success(void){
    if(PDO_Len != 0 && PD_Ctl.PD_State == STA_IDLE)
        return 1;
    else
        return 0;
}
